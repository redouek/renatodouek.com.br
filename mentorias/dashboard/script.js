// Arquivo: mentorias/dashboard/script.js (COMPLETO E CORRIGIDO)
...
// getStatusClass agora define classes para o BADGE (Item 3)
...
function renderActivities(activitiesToRender) {
    ...
    activitiesToRender.forEach(activity => {
        ...
        // LOGS DE DEBUG PARA DESCRIÇÃO (Item 9)
        console.log("Atividade a ser renderizada:", activity);
        console.log("Valor de 'atividade.descricaoObservacoes':", activity.descricaoObservacoes);
        console.log("Valor final injetado para descrição:", String(activity.descricaoObservacoes || 'VAZIO_NO_OBJETO_OU_CAMPO'));


        row.innerHTML = `
            <td><input type="checkbox" data-activity-id="${activity.IDdaAtividade}" <span class="math-inline">\{isCompleted ? 'checked' \: ''\}\></td\>
<td\></span>{activity.Atividade || ''}</td>
            <td><span class="math-inline">\{activity\.descricaoObservacoes \|\| ''\}</td\> <td\></span>{formatarDataParaExibicao(activity.DataLimite)}</td>
            <td>
                <span class="status-badge <span class="math-inline">\{getStatusClass\(activity\.StatusAtual\)\}" data\-activity\-id\="</span>{activity.IDdaAtividade}" title="Clique para editar status"><span class="math-inline">\{activity\.StatusAtual\}</span\>
</td\>
<td\>
<button class\="edit\-activity\-btn icon\-action\-btn" data\-activity\-id\="</span>{activity.IDdaAtividade}" title="Editar">
...
function filterAndSearchActivities() {
    let filtered = allActivities.filter(activity => {
        ...
        return (
            ...
            String(activity.descricaoObservacoes || '').toLowerCase().includes(searchTermLower) // Usando a chave normalizada
        );
    });
...
function showNotification(message, showUndo = false) {
    ...
    if (showUndo) {
        ...
        progressLine.style.animation = 'progressAnimation 3s linear forwards'; // MUDANÇA: 3 segundos (Item 6)
        ...
        }, 3000); // MUDANÇA: 3 segundos (Item 6)
    } else {
